...
class ProdutoController:
    ...
    def index():
        ...
        @produto_controller.route('/produtod/<id>', methods=['GET'])
        def get_produto(id):
            produto = Produto.query.get(id)
            produto_schema = ProdutoSchema()
            produtos = produto_schema.duap(produto)

            return make_response(jsonify( {

            }))

        @produto_controller.route('/produto/<id>', methods=['POST'])
        def create()
        ...
        def delete(id):
            ...
            db.session.commit()
            return make response(jsonify({}),204)

        @produto_controller.route('/produto/<id>', methods=['PUT'])
        def update(id):
